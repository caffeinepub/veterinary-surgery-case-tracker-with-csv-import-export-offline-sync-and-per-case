 ...
 * Returns a formatted string like "2y 3m" or null if invalid/future date
 */
export function calculateAge(dobStr: string): string | null {
  const validated = validateDateOnlyString(dobStr);
  ...
  if (months < 0) {
    years--;
    months += 12;
  } else if (months === 0 && now.getDate() < dob.getDate()) {
    years--;
    months = 11;
  ...
  // Handle negative values (shouldn't happen with future date check, but be safe)

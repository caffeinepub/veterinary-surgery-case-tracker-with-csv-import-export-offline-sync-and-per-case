...
/**
 * Grants the caller '#user' permission by calling initUserPrivileges.
 * After success, actively refetches profile and server cases queries to ensure immediate UI update.
 */
export function useGrantUserAccess() {
  ...
  return useMutation({
    mutationFn: async () => {
      if (!actor) throw new Error('Backend connection not available');
      return await actor.initUserPrivileges();
    },
    onSuccess: async () => {
      // Actively refetch (not just invalidate) all queries that require user permission
      // This ensures immediate UI update without requiring manual retry
      await Promise.all([
        queryClient.refetchQueries({ queryKey: ['currentUserProfile'] }),
        queryClient.refetchQueries({ queryKey: ['serverSurgeryCases'] }),
        queryClient.refetchQueries({ queryKey: ['mergedCases'] }),
      ]);
    },
  });
}
